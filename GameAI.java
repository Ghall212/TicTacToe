...
public class GameAI
{
	...
	{
			ArrayList<GameState> children = state.generateSuccessors(state, AI_LETTER);
			
			int best = Minimax.miniMax(children.get(0), "X");
			int size = children.size();
			int updateState = 0;
			
			for(int i =1; i<size; i++)
			{
				if(best < Minimax.miniMax(children.get(i), "X"))
				{
					best = Minimax.miniMax(children.get(i), "X");
					Log.debug("Selected State Value tmpwinnerVal " + best);
					updateState = i;
				}
			}
			
			boardState = children.get(updateState).getBoardState();
